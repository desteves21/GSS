---
title: "General Social Survey"
author: "Daniel Esteves"
date: "Summer 2024"
output: 
  html_document:
    number_sections: true
    toc: true
---


```{r include=FALSE,echo=FALSE}
require(tigerstats)
require(tidyverse)
require(foreign)
source("GrabData.R")
```



#  Question 1

<!--In this section you explain what you are trying to show.  Where did the data come from?  What is the research or other question you are trying to answer?!-->

The data in the dataframe GSSdata is from the 2018 General Social Survey. The first blocks of R-code has selected down a subset of the data to just 16 variables. It has further removed unwanted factor levels in much of the data. Examine the code in the GrabData.R file to see what it is doing. Some of the variables are categorical and others are numerical. Be sure to do a variable analysis before tackling each question.  
First question - Is opinion on the death penalty (CAPPUN) independent of gun ownership (OWNGUN)?

$H_0$: CAPPUN is independent of OWNGUN

$H_0$: CAPPUN is not independent of OWNGUN

## Methods

<!--Decide on your methods:  use "variable analysis" or other appropriate descriptors.  Make sure to choose at least one graphical method and at least one numerical method.!-->

CAPPUN is a categorical variable

OWNGUN is a categorical variable, so I use the cat~cat helper file

## Results

<!--Divide this section into two sub-sections:  One for your descriptive  results and one for your inferential results.!-->

### Descriptive Results

#### Graphical Descriptive Results

```{r}
barchartGC(~CAPPUN + OWNGUN,data=GSSdata)
barchartGC(~CAPPUN + OWNGUN,data=GSSdata, type="percent")

```

This graph shows that half of gun owners are in favor of guns, while more than half of non-gun owners are not in favor of them.  

<!--Graphical results here.  Make sure to show your code.  Provide appropriate labels for axes, giving units if possible, and provide a good title for the graph, too.  Use the graphical results to describe the patterns if any that exist in the data as focused toward the research question!-->

#### Numerical Descriptive Results

<!--Numerical results go here. Use the numerical results to describe the patterns if any that exist in the data as focused toward the research question!-->

```{r}
table1 <- xtabs(~CAPPUN + OWNGUN,data=GSSdata)
rowPerc(table1)
colPerc(table1)
```

This table shows that the relationship between gun ownership and opinions on capital punishment, indicate that a higher percentage of gun owners favor capital punishment compared to non-gun owners.  

  
### Inferential Results

<!--State hypothesis clearly.  Make sure your discussion of the inferential test covers all the aspects that the test output produces, such as test statistic, p-value etc.  Make a decision about the null hypothesis, explain the assumptions on which the selected test/procedure was based, and why the chosen procedure satisfys the assumptions and is appropriate to answer the research question!-->

```{r}
chisq.test(table1)
chisqtestGC(table1)
fisher.test(table1)
```

The p-value for the Chi-squared test was p=0.0202, this means we reject the null hypothesis, and conclude that the opinion on capital punishment(CAPPUN) is not independent of gun ownership(OWNGUN)  

The p-value for the fisher exact test was p=0.01651, and the odds ration was 2.271:1
the odds ratio, 2.271:1 means that people do no own a gun are 2.271 times more likely to oppose capital punishment(CAPPUN)  


#  Question 2

<!--In this section you explain what you are trying to show.  Where did the data come from?  What is the research or other question you are trying to answer?!-->

Is there a difference in years of education (EDUC) between the those who did or did not vote in 2012 (VOTE12)?



### Introduction 
$H_0$: There is no difference of years EDUC who did not VOTE12  

$H_A$: There is a difference of years EDUC who did VOTE12  


## Methods

<!--Decide on your methods:  use "variable analysis" or other appropriate descriptors.  Make sure to choose at least one graphical method and at least one numerical method.!-->

EDUC is numerical 

VOTE12 is a factor, so I use the helper file Num~Cat 



##Results
<!--Divide this section into two sub-sections:  One for your descriptive  results and one for your inferential results.!-->

### Descriptive Results

#### Graphical Descriptive Results

<!--Graphical results here.  Make sure to show your code.  Provide appropriate labels for axes, giving units if possible, and provide a good title for the graph, too.  Use the graphical results to describe the patterns if any that exist in the data as focused toward the research question!-->

```{r}
plot(EDUC~VOTE12,data=GSSdata)
```

This boxplot shows that people who voted in 2012 generally had more years of education (EDUC) compared to those who did not vote in 2012.  

#### Numerical Descriptive Results

<!--Numerical results go here. Use the numerical results to describe the patterns if any that exist in the data as focused toward the research question!-->

```{r}
favstats(EDUC~VOTE12,data=GSSdata)
```

The numerical results indicate that in 2012, people who voted had an average of nearly 16 years of education, while those who did not vote had just over 13 years of education.

### Inferential Results

<!--State hypothesis clearly.  Make sure your discussion of the inferential test covers all the aspects that the test output produces, such as test statistic, p-value etc.  Make a decision about the null hypothesis, explain the assumptions on which the selected test/procedure was based, and why the chosen procedure satisfys the assumptions and is appropriate to answer the research question!-->

```{r}
t.test(EDUC~VOTE12,data=GSSdata)
```


This t-test shows that people who voted in 2012 had more years of education. The p-value is 0.0000001248, which is much smaller than 0.05. Because of this, we reject the null hypothesis, meaning there is a significant difference in the number of years of education between those who voted and those who didn't.

#  Question 3

<!--In this section you explain what you are trying to show.  Where did the data come from?  What is the research or other question you are trying to answer?!-->

Determine if a persons performance on the wordsum test (WORDSUM) is independent of their level of schooling (DEGREE).


### Introduction 
$H_0$: The score of a persons wordsum test is independent of their schooling

$H_A$: The score of a persons wordsum test is not independent of their schooling

## Methods

<!--Decide on your methods:  use "variable analysis" or other appropriate descriptors.  Make sure to choose at least one graphical method and at least one numerical method.!-->

WORDSUM is numerical

DEGREE is categorical, so I use the helper file Num~Cat


##Results

<!--Divide this section into two sub-sections:  One for your descriptive  results and one for your inferential results.!-->

### Descriptive Results

#### Graphical Descriptive Results

<!--Graphical results here.  Make sure to show your code.  Provide appropriate labels for axes, giving units if possible, and provide a good title for the graph, too.  Use the graphical results to describe the patterns if any that exist in the data as focused toward the research question!-->

```{r}
boxplot(WORDSUM~DEGREE,data=GSSdata)
```

The boxplot shows that as the education level goes up from less than high school to graduate, the median WORDSUM scores also increase. Graduates generally have higher WORDSUM scores compared to people with lower education levels.  


#### Numerical Descriptive Results

<!--Numerical results go here. Use the numerical results to describe the patterns if any that exist in the data as focused toward the research question!-->

```{r}
favstats(WORDSUM~DEGREE,data=GSSdata)
```

The data shows that as people get more education, their WORDSUM scores usually go up. Graduates have the highest average and median scores, followed by those with Bachelor's degrees, then Junior College degrees, then High School degrees, and lastly, those with less than a High School education. This suggests that more education leads to better WORDSUM scores.  

### Inferential Results

<!--State hypothesis clearly.  Make sure your discussion of the inferential test covers all the aspects that the test output produces, such as test statistic, p-value etc.  Make a decision about the null hypothesis, explain the assumptions on which the selected test/procedure was based, and why the chosen procedure satisfys the assumptions and is appropriate to answer the research question!-->

```{r}
anova=aov(WORDSUM~DEGREE,data=GSSdata)
summary(anova)
```

The data shows that we reject the null hypothesis because the p-value is 0.00000025, which is much smaller than 0.05. This indicates a difference in WORDSUM scores across different education levels.


```{r}
#we specify the margins starting at the bottom and proceeding clockwise - thus 8.7 is the left side margin to hold the names of the factors

par(mar = c(7, 8.7, 2, 2))
plot(TukeyHSD(anova),las=2)
```

There is no difference between high school and less than high school.  
There is a difference between junior college and high school.   
There is a difference between bachelor college and high school.  
There is a difference between graduate and high school.  
There is no difference between highschool and graduate.  
There is no difference between junior college and graduate.  
There is no difference between bachelor and graduate.  

# Discussion and Conclusion

<!--Discussion and conclusion here.  If you found a relationship be sure to consider whether the relationship occurs because one of the variavbles causes the other, or whether they perhasps are related for some other reason.  Watch the chapter 6 videos from the GeorgeTown videos collection.!-->

#### Question 1: Is gun ownership independant of opinion on captal punishment. 

The summary and inferential results indicate that gun ownership and opinions on capital punishment are not independent. 

#### Question 2: Is there a difference in years of education (EDUC) between the those who did or did not vote in 2012 (VOTE12)?

The summary and inferential results show a difference in the years of education between people who voted in 2012 and those who didn't.

#### Question 3: Is a persons performance on the wordsum test (WORDSUM) independent of their level of schooling (DEGREE)?

The summary and inferential results suggest that performance on the WORDSUM test is not independent of a person's level of schooling.